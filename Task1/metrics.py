...
def compute_bertscore(
    ...
    # Guard: refs rỗng → dùng chuỗi rỗng
    flat_references = [refs[0] if refs else "" for refs in references_list]

    valid_pairs = [
        (p, r) for p, r in zip(predictions, flat_references)
        if p.strip() and r.strip()
    ]

    if not valid_pairs:
        print("[BERTScore] Cảnh báo: tất cả predictions rỗng — trả về 0.0")
        return {
    ...
    if n_valid < n_total:
        print(f"[BERTScore] Bỏ qua {n_total - n_valid} cặp rỗng, tính trên {n_valid}/{n_total} mẫu")
...
def compute_llm_judge(
    ...
    provider: str = "gemini",   # "gemini" hoặc "openai"
    max_samples: int = 100,
) -> Dict[str, float]:
    """
    LLM-as-a-Judge — hỗ trợ 2 provider:
      - "gemini" : dùng google/gemini-1.5-flash
    ...
    if provider == "gemini":
        ...
        def _call(prompt: str) -> int:
            return int(llm.generate_content(prompt).text.strip())

    elif provider == "openai":
        if openai_api_key is None:
            print("[LLM Judge] Bỏ qua — không có OpenAI API key.")
            return {}
        try:
            from openai import OpenAI
        except ImportError:
            print("[LLM Judge] Thiếu openai. Chạy: pip install openai")
            return {}
        ...
        print(f"[LLM Judge] Provider không hợp lệ: {provider}. Chọn 'gemini' hoặc 'openai'.")
        return {}

    scores = []
    n = min(len(predictions), max_samples)
    print(f"[LLM Judge] Đang chấm {n} mẫu bằng {provider.upper()}...")

    for i in range(n):
        prompt = LLM_JUDGE_PROMPT.format(
            question=questions[i],
            reference=references_list[i][0] if references_list[i] else "",
    ...
    print(f"[LLM Judge] Hoàn tất! Điểm trung bình: {avg}/5.0 ({n} mẫu)")
...
def generate_predictions(
    ...
    Dùng trong evaluate_model_on_testset() để đảm bảo normalize nhất quán giữa prediction và ground truth
    Returns:
...
def evaluate_all(
    ...
    if use_llm_judge and questions is not None:
        print("→ Chạy LLM-as-a-Judge...")
        results.update(compute_llm_judge(
            predictions, ground_truths_list, questions,
...
def compare_configs(results_list: List[Dict]) -> None:
    ...
    if not results_list:
        print("[compare_configs] Chưa có kết quả nào để so sánh.")
        return
