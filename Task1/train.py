...
def train_model():
    ...
    # ── Import model ─────────────────────────────────────────────────────────────
    if args.model == 'a1':
        from lstm_dec import VQA_A1_Model
    else:
        from trans_dec import VQA_Generative_Model
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            no_improve += 1
            print(f"Khong cai thien ({no_improve}/{PATIENCE})")
            if no_improve >= PATIENCE:
                print(f"Early stopping tai epoch {epoch+1}!")
                break

    print(f"\nHuan luyen hoan tat! Best Val Loss: {best_val_loss:.4f}")
